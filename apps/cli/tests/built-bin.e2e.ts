...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    ...
    try {
      ...
      const nonNumericPort = await runBuiltBin(['web', '--port', 'abc'], {
        DSH_HOME: home,
        DSH_TELEMETRY_DISABLED: '1',
      })
      expect(nonNumericPort.code).toBe(1)
      expect(nonNumericPort.stdout).toBe('')
      expect(nonNumericPort.stderr).toContain('--port must be a number')
      expect(nonNumericPort.stderr).not.toContain('dsh web: http://')
